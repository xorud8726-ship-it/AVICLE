...
def add_to_cart():
    ...
    for name, add_qty in items_to_add:
        for child in cart_tree.get_children():
            ...
            if tree_name == name:
                cart_tree.item(child, values=(name, int(tree_qty) + add_qty))
                break
        else:
...
def save_order_to_txt(order_list):
    ...
    filename = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S_%f") + "_발주내역.txt"
...
def open_item_image(event):
    ...
    if not selected:
        return
    name, _ = cart_tree.item(selected[0], "values")
    if name not in item_images:
        messagebox.showinfo("이미지 없음", "이미지가 없습니다.")
        return
    filepath = find_image_file(item_images[name])
    if filepath is None:
        messagebox.showinfo("이미지 없음", "이미지가 없습니다.")
        return
    try:
...
def show_notice():
    ...
    win.geometry(f"550x420+{x}+{y}")
    win.grab_set()

    tk.Label(win, text=text, font=("Helvetica", 12), justify="left", wraplength=520).pack(padx=10, pady=10)
    tk.Button(win, text="확인", command=win.destroy, font=("Helvetica", 12)).pack(pady=10)
